        ...
        <div class="container mx-auto flex justify-between items-center px-4">
            ...
            <nav>
                <ul class="flex space-x-6">
                    ...
                    <li><a href="#galeri-foto" class="hover:opacity-80">Galeri Sekolah</a></li>
                </ul>
            </nav>
        </div>
    </header>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-8">
                        ...
                        <div
                            ...
                            <div class="p-6">
                                <h3 class="text-xl font-semibold text-gray-800 mb-4">{{ $profile->judul }}</h3>
                                <p class="text-gray-600 leading-relaxed">{{ $profile->isi }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
...
<div id="informasi-terkini" class="bg-white py-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
            <div class="p-6">
                <h3 class="text-xl font-semibold text-gray-800 mb-4">{{ $post->judul }}</h3>

                <!-- Menampilkan Foto -->
                @php
                    $galeri = \App\Models\Galery::where('post_id', $post->id)->first();
                    $foto = $galeri ? \App\Models\Foto::where('galery_id', $galeri->id)->first() : null;
                @endphp
                ...
                <!-- Text yang disingkat -->
                <p class="text-gray-600 mb-4" id="short-text-{{ $post->id }}">
                    {{ Str::limit($post->isi, 150) }}
                    <button onclick="toggleText({{ $post->id }})" class="text-blue-600 hover:text-blue-700 font-medium">
                        Baca selengkapnya →
                    </button>
                </p>

                <!-- Text lengkap (hidden by default) -->
                <p class="text-gray-600 mb-4 hidden" id="full-text-{{ $post->id }}">
                    {{ $post->isi }}
                    <button onclick="toggleText({{ $post->id }})" class="text-blue-600 hover:text-blue-700 font-medium">
                        Tutup ←
                    </button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div
                        ...
                        <div class="p-6">
                            <h3 class="text-xl font-semibold text-gray-800 mb-4">{{ $post->judul }}</h3>
                            <!-- Text yang disingkat -->
                            <p class="text-gray-600 mb-4" id="short-agenda-{{ $post->id }}">
                                {{ Str::limit($post->isi, 150) }}
                                <button onclick="toggleAgenda({{ $post->id }})"
                                    class="text-blue-600 hover:text-blue-700 font-medium">
                                    Baca selengkapnya →
                                </button>
                            </p>
                            <!-- Text lengkap (hidden by default) -->
                            <p class="text-gray-600 mb-4 hidden" id="full-agenda-{{ $post->id }}">
                                {{ $post->isi }}
                                <button onclick="toggleAgenda({{ $post->id }})"
                                    class="text-blue-600 hover:text-blue-700 font-medium">
                                    Tutup ←
...
<div id="galeri-foto" class="bg-gray-50 py-20">
    <h2 class="text-3xl font-bold text-center mb-8 text-black">Galeri Sekolah</h2>

     <!-- Album List -->
     <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @foreach ($galeries as $postTitle => $gallery)
        <div class="album-card" onclick="openAlbum('album-{{ Str::slug($postTitle) }}')">
            <!-- Album Cover -->
            <div class="relative group cursor-pointer bg-white rounded-xl shadow-md overflow-hidden">
                <!-- Cover Image -->
                <div class="aspect-[4/3] overflow-hidden">
                    @if($gallery->first() && file_exists(storage_path('app/public/' . $gallery->first()->file)))
                    <img src="{{ asset('storage/' . $gallery->first()->file) }}" alt="{{ $postTitle }}"
                    ...
                    <div
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-4">
                            <p class="text-white text-sm">{{ $gallery->count() }} Foto</p>
                        </div>
                    </div>
                </div>
            ...
            <!-- Album Modal -->
            <div id="album-{{ Str::slug($postTitle) }}"
                class="fixed inset-0 z-50 hidden bg-black/95 backdrop-blur-sm">
                <div class="min-h-screen p-4 md:p-8" onclick="event.stopPropagation();">
                    <div class="relative max-w-7xl mx-auto">
                        <!-- Header -->
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-2xl md:text-3xl font-bold text-white">{{ $postTitle }}</h3>
                            <button type="button"
                                onclick="event.stopPropagation(); closeAlbum('album-{{ Str::slug($postTitle) }}')"
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            ...
                            <div class="group relative aspect-square overflow-hidden rounded-lg cursor-pointer"
                                onclick="event.stopPropagation(); openLightbox('{{ asset('storage/' . $foto->file) }}', '{{ $foto->judul }}')">
                                @if(file_exists(storage_path('app/public/photos/' . $foto->file)))
            ...
            <div class="border-t border-gray-700 pt-8">
                <div class="flex flex-col md:flex-row justify-between items-center">
                    ...
                    <p class="text-gray-400 text-sm">
                        &copy; {{ date('Y') }} Syaziliya. All Rights Reserved.
                    </p>
                </div>
            </div>
        </div>
